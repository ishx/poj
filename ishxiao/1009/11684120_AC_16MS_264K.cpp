...
int nbuffer = 0; // number of wide characters stored in buffer
...
{ 
	...
	long minAfter = NMAX*2; // 2000000000  
	for (int r = 0; r < 3; r++)
	{  
		if (firstInRun(r))
			return 1; 
		if (minAfter > leftInRun[r])
			minAfter = leftInRun[r]; 
	} 
	if (minAfter == 0)
		return 1;
...
{  
   ...
   while (imageWidth > 0) // assume end sentinal is 0 image width
   {
		...
		lastRunIndex = 0;
		runLength[lastRunIndex] = 2*imageWidth; // add initial border  
		do { 
			lastRunIndex++; 
		...
		//cout << imageWidth << endl; //output imagewidth
		nbuffer += sprintf(buffer+nbuffer,"%d\n",imageWidth);
		...
		if (column != 0)
			cout << " not multiple of width " << imageWidth << endl; 
